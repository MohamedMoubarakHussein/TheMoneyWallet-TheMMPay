...
public class eventListener {
    ...
    public void handleSignupEvent(Event eventz) {
        ...
        HashMap<String, String> prefernce = (HashMap<
                String,
                String
            >) user.remove("preferences");
        user.remove("preferences");
        UserSingUpEvent userSingUpDetails = objectMapper.convertValue(
            user,
            UserSingUpEvent.class
        );

        UserPreferences userPreferences = objectMapper.convertValue(
            prefernce,
        ...
        try {
            ...
            event2.setAdditionalData(
                Map.of("data", Map.of("data", profile.toString()))
            );
